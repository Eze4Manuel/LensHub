...
import { HEAD_API_BASE_URL, HEAD_APP_URL } from '../../config';
...
export default class Reviewer extends React.Component {
  state = {
    ...
    componentDidMount (){
      ...
      //Fetching Data for Profile Completion
       $.ajax({ url: HEAD_API_BASE_URL + '/getHeadReviewingProducts/?skip='+ this.state.skip +'&take='+ this.state.take, method: "GET", })
     ...
     handleFetchMore = (e) => {
       //Fetching Data for Profile Completion
        ...
        $.ajax({
          url: HEAD_API_BASE_URL + '/getHeadReviewingProducts/?skip='+ this.state.skip +'&take='+ this.state.take, method: "GET",})
  ...
  render(){
    return (
      <>{(this.state.products.length > 0) ?
        <Reviewing products= {this.state.products} loader = {this.state.loader} handleFetchMore = {this.handleFetchMore} handleRecheck={this.handleRecheck} /> :
 ...
 function Reviewing(props) {
  ...
  const handleApprove = (id, ind) => {
    //Fetching Data for Profile Completion
      setLoader(true);
     $.ajax({ url: HEAD_API_BASE_URL + '/setHeadApprove/' + id, method: "POST" })
  ...
  const looper =   props.products.map( (val, ind) => (
       <div className="card horizontal" key={ind}>
        ...
        <div className="card-stacked">
          ...
          <a href="#" onClick={()=>{handleApprove(val.product_id, ind)}}>Approve</a>
          <a href="#" onClick={()=>{handleDecline(val.product_id, ind)}}>Decline</a>
