...
componentDidMount (){
  //Fetching Data for Profile Completion
   $.ajax({
     url: API_BASE_URL + '/getReviewingProducts/'+ USER_ID+'?skip='+ this.state.skip +'&take='+ this.state.take,
     method: "GET",
   })
   .done( res => {
  ...
  render(){
    return (
      <>{(this.state.products.length > 0) ?
        <Reviewing products= {this.state.products} loader = {this.state.loader} handleFetchMore = {this.handleFetchMore} /> :
